...
public static class HostBuilderExtensions
{
    public static IHostApplicationBuilder AddDemo(this IHostApplicationBuilder builder)
    {
        ...
        var azureAISearchConfiguration =
            builder
                .Configuration.GetRequiredSection(nameof(AzureAISearchConfig))
                .Get<AzureAISearchConfig>()
            ?? throw new NullReferenceException($"Missing {nameof(AzureAISearchConfig)}");
...
#pragma warning disable SKEXP0020, CS0612
        kernelBuilder.AddAzureAISearchVectorStore(
            new Uri(azureAISearchConfiguration.Endpoint),
            new AzureKeyCredential(azureAISearchConfiguration.ApiKey));
#pragma warning restore SKEXP0020, CS0612
